function samples = fsong( score )
  ...
  % Notes decay in volume over time, creating fading sound
...
function rv = tone(duration, freq)
  ...
  %fade each note by decreasing amplitude over time
  rv = exp(-10*sampleTimes) .* sin(2*pi*freq*sampleTimes) *.999; % scale
end
